...
def process_frame(frame, num_points_avg):
    ...
    def pass_red(red_contours):
        ...
        for contour in red_contours:
            ...
            if y+h > int(height*pass_threashold):
                ...
                if (x+w) > left_region:
                    left_region = (x+w) + 5 #pixels old value: 30
    ...
    if contours_detected:
        ...
        if time.time() - last_contour_time < 1.3: # 0.9 is the time to keep the track after obstacle passed
            clearance_pass = True
            flage = "obs clearance"
