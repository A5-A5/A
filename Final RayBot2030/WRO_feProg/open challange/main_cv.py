...
class stuck_checker:
    ...
    def check_if_stuck( self, frame, similarity_threshold = 0.95, similarity_duration = 3):
...
def main():
    ...
    servo = create_servo(channel=0, max_pulse=2500, min_pulse=500, reverse_angle=True, max_angle=40)
    ...
    lab_count = 0
    start_counter = True
    num_points_avg = 90 # How many points from the trace to avg
    steerLock_duration = 0
    stuck_checker_obj = stuck_checker()
    steer_angle = 0
    steer_angle_buffer = collections.deque(maxlen=3)
    run_speed = 0.6
    offset_fix_factor = 2 # to compensate quircky track , rais above 0 if bad ccw and lower for bad cw
    try:
        while True:
            curr_time = time.time()
